...
function addObjects (containerElem, jtree) {
  ...
  for (key in jtree) {
    ...
    */
    td.innerHTML = caret+' <i class="icon '+icon+' "></i> '+jo.name;
    td.style.cursor = 'pointer';
    td.style.paddingLeft = (level+padding)+'em';
    ...
    td = document.createElement('td');
    td.classList.add('td1');
    td.innerHTML = secondsToString(jo.updateTime);
    tr.appendChild(td);
    ...
    tr.setAttribute('type', jo.type);
    tr.setAttribute('name', jo.name);    
    tr.oncontextmenu = createContextMenu;
...
function refreshTable () {
  ...
  var header = ['Name', 'Status', 'Modified'];
  ...
  callServer ("GET", "/catalog", null, function (xhttp) {
      if (xhttp.readyState == 4) {
        if (xhttp.status == 200) {
          ...
          document.getElementById('catalog-table').innerHTML = xhttp.statusText;
        }
      }
      else if (this.readyState == 3) { // Processing request 
        document.getElementById('catalog-table').innerHTML = "Getting blueprints...";
      }
      else {
        document.getElementById('catalog-table').innerHTML = xhttp.statusText;
      }
